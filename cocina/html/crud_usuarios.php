...
<head>
    ...
    <title>Ingreso nuevo de usuarios</title>
...
<input type="checkbox" id="nav-toggle">
    ...
    <div class="container">
        <h2 class="text">Ingresar Nuevo Usuario</h2>
        <form action="insert_user.php" method="POST">
            <input type="text" name="nombre_usuario" placeholder="Nombres del usuario" required>
            ...
            <select name="rol" id="rol" class="select" required>
                ...
                <?php
                    $query = "SELECT * FROM usuarios_nivel";
                    $result = mysqli_query($con, $query) or die(mysqli_error($con));
                    while ($row = mysqli_fetch_array($result)) {
                        echo '<option value="'.$row['nivel_usuario'].'">'.$row['nivel_usuario'].'</option>';
                    }
                ?>
            </select>

            <input type="text" name="id_nivel" placeholder="Nivel de usuario" required>
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td>".$row['rol']."</td>";
            echo "<td>".$row['id_nivel']."</td>";
            echo "<td>".$row['pass']."</td>";
            echo "<td>";
            echo "<div class='grid-btns-crud'>";
            echo "<div><a href='update_user.php?id=".$row['id']."' class='bg-teal-500 p-1.5 rounded-[4px]'>Editar</a></div>";
            echo "<div><a href='delete_user.php?id=".$row['id']."' class='bg-rose-500 p-1.5 rounded-[4px]'>Eliminar</a></div>";
            echo "</div>";
            echo "</td>";
            echo "</tr>";
        }
