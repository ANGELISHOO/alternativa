...
<input type="checkbox" id="nav-toggle">
    ...
    <div class="container">
    <h2 class="text">Ingreso nuevo traspaso</h2>
        <form action="insert_traspaso.php" method="POST">
            <input type="text" name="elemento" placeholder="Elemento" required>
            <input type="number" name="baja" placeholder="Baja" required>
            <input type="date" name="fecha" placeholder="Fecha" required>
            <select name="afectada" id="afectada" class="select" required>
                <option value="">Afectada</option>
                <option value="">VERDADERO</option>
                <option value="">FALSO</option>
                <?php
                    $query = "SELECT * FROM afectada";
                    $result = mysqli_query($con, $query) or die(mysqli_error($con));
                    while ($row = mysqli_fetch_array($result)) {
                        echo '<option value="'.$row['afectada'].'">'.$row['afectada'].'</option>';
                    }
                ?>
            </select>
    ...
    $sql = "SELECT * FROM traspaso WHERE elemento LIKE '%$searchTerm%' OR fecha LIKE '%$searchTerm%' OR afectada LIKE '%$searchTerm%'";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<div><a href='update_traspaso.php?id=".$row['id']."' class='bg-teal-500 p-1.5 rounded-[4px]'>Editar</a></div>";
            echo "<div><a href='delete_traspaso.php?id=".$row['id']."' class='bg-rose-500 p-1.5 rounded-[4px]'>Eliminar</a></div>";
            echo "</div>";
            echo "</td>";
            echo "</tr>";
        }
    } else {
        echo "<tr><td colspan='7'>No se encontro ningun traspaso</td></tr>";
    }
    ?>
</tbody>
            </table>
