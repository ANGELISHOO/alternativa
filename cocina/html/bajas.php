...
<head>
    ...
    <title>Ingreso nueva Baja</title>
...
<input type="checkbox" id="nav-toggle">
    ...
    <div class="container">
    <h2 class="text">Ingresar Baja</h2>
        <form action="insert_bajas.php" method="POST">
        <select name="elemento" id="elemento" class="select" required>
        <option value="">Seleccione un elemento</option>
                <?php
                    // Consulta para obtener los elementos de la tabla "elementos"
                    $query = "SELECT * FROM inventario";
                    $result = mysqli_query($con, $query) or die(mysqli_error($con));

                    // Generar las opciones del select con los elementos obtenidos
                    while ($row = mysqli_fetch_array($result)) {
                        echo '<option value="'.$row['elementos'].'">'.$row['elementos'].'</option>';
                    }
                ?>
            </select>           
            <input type="number" name="baja" placeholder="Cantidad de bajas" required>
    ...
    $sql = "SELECT * FROM bajas WHERE elemento LIKE '%$searchTerm%' OR fecha LIKE '%$searchTerm%' OR afectada LIKE '%$searchTerm%'";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td>".$row['afectada']."</td>";
            echo "<td>";
            echo "<div class='grid-btns-crud'>";
            echo "<div><a href='update_bajas.php?id=".$row['id']."' class='bg-teal-500 p-1.5 rounded-[4px]'>Editar</a></div>";
            echo "<div><a href='delete_bajas.php?id=".$row['id']."' class='bg-rose-500 p-1.5 rounded-[4px]'>Eliminar</a></div>";
            echo "</div>";
            echo "</td>";
            echo "</tr>";
        }
    } else {
        echo "<tr><td colspan='7'>No se encontro la baja</td></tr>";
    }
    ?>
</tbody>
            </table>
