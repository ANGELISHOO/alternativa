...
<head>
    ...
    <title>Ingreso nuevo instructor</title>
...
<input type="checkbox" id="nav-toggle">
    ...
    <div class="container">
        <h2 class="text">Ingresar Nuevo Instructor</h2>
        <form action="insert_docente.php" method="POST">
            <input type="text" name="nombre" placeholder="Nombres del instructor" required>
        ...
        <h2 class="text">Instructores Registrados</h2>
    ...
    $sql = "SELECT * FROM docentes WHERE nombre LIKE '%$searchTerm%' OR curso LIKE '%$searchTerm%' OR activo LIKE '%$searchTerm%'";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<div><a href='update_docente.php?id=".$row['id']."' class='bg-teal-500 p-1.5 rounded-[4px]'>Editar</a></div>";
            echo "<div><a href='delete_docente.php?id=".$row['id']."' class='bg-rose-500 p-1.5 rounded-[4px]'>Eliminar</a></div>";
            echo "</div>";
            echo "</td>";
            echo "</tr>";
        }
